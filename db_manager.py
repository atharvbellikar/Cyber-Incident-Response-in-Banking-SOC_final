...
# ─────────────────────────────────────────
# User & Session Auth Functions
# ─────────────────────────────────────────

def init_users_db():
    """Create users and sessions tables if they don't exist. Seeds a default admin."""
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    if cursor.fetchone()[0] == 0:
        ...
        conn.commit()
        print("[db_manager] Seeded default admin user (username: admin, password: admin123)")

    conn.close()


def create_user(username: str, password_hash: str, role: str = "analyst") -> dict:
    from datetime import datetime, timezone
    conn = get_db_connection()
    cursor = conn.cursor()
    created_at = datetime.now(timezone.utc).isoformat()
...
def create_session(user_id: int, token: str) -> None:
    ...
    created_at = datetime.now(timezone.utc).isoformat()
    cursor.execute(
        "INSERT OR REPLACE INTO sessions (token, user_id, username, role, created_at) VALUES (?, ?, ?, ?, ?)",
        (token, user_id, row["username"], row["role"], created_at)
    )
    conn.commit()
    conn.close()
